# 日计划索引

日计划用于把每天的学习任务打包成一个可执行清单。每个 Day 页面都应同时链接到 LeetCode 任务、技术学习任务、代码产物和复盘要求。

## 当前进度

- Current Day: Day 1 / 36
- Week Day: Week 1 Day 1
- 状态：任务包已准备，等待学习完成确认

## Day 列表

| Day | Week Day | LeetCode | 技术学习 | 状态 |
| --- | --- | --- | --- | --- |
| [Day 1](/days/day-01) | Week 1 Day 1 | [数组 + 哈希专项](/leetcode/day-01-arrays-hash) | [最小二乘与梯度下降](/notes/week-01/day-01-least-squares-gradient-descent) | 已准备 |

## 每日完成标准

- LeetCode 完成当天 `3-5` 题，并能复述核心模式。
- 技术学习读完当天章节，完成公式推导、代码映射和自测题。
- 如果当天有代码产物，至少运行一次最小验证命令。
- 复盘当天最容易错的 3 个点，写入对应 Day 页面或专题页。
